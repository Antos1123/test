...
public class Instruments extends JavaPlugin {
	...
	private void loadConfig() {
		...
		if (!isExists) {
			...
			config.set("worldguard.place", List.of("instrument_"));
			config.set("worldguard.message", "&c해당 장소에서는 악기를 연주할 수 없습니다");
			config.set("worldguard.sound.sound-name", "minecraft:block.anvil.place");
			config.set("worldguard.sound.volume", 1.0);
			config.set("worldguard.sound.pitch", 1.0);
		}
	...
	private void addBukkitRecipes() {
		for (String instrumentKey : config.getConfigurationSection("settings.instruments.recipe").getKeys(false)) {
			...
			for (String ingredientKey : config.getConfigurationSection(ingredientsPath).getKeys(false)) {
				Material material = Material.valueOf(config.getString(ingredientsPath + "." + ingredientKey));
